    ...
    if request.method == "POST":
        ...
        if "IsLogin" in data:
            if validate_login(request.json):
                ...
                return "Success", 200
            diagnostics.log_login_attempt(request.remote_addr, False)
            return "Invalid credentials", 403
        elif "Is2FA" in data:
            if not two_fa.is_code_valid(data.get("TwoFA", "")):
                diagnostics.log_exploit_attempt(request.remote_addr, "Invalid 2FA code", request.user_agent.string)
                return "2FA code expired", 401  # Give false impression that code expired instead of 2FA being wrong.
            diagnostics.log_login_attempt(request.remote_addr, True)
        ...
        diagnostics.log_exploit_attempt(request.remote_addr, "Invalid method on /admin", request.user_agent.string)
...
def proxy_page(dst: str):
    if dst != escape(dst):
        diagnostics.log_exploit_attempt(request.remote_addr, f'Invalid URL: "{dst}"', request.user_agent.string)
        return "Invalid URL", 404

    if not validate_url(dst):
        diagnostics.log_exploit_attempt(request.remote_addr, f'Non-Roblox URL: "{dst}"', request.user_agent.string)
    ...
    # Prefix with warning just in case Roblox expects it.
    ...
    return response if successful else (response, 500)
